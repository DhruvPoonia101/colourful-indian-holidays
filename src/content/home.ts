...
export const introCopy = {
  ...
  paragraphs: [
    "Colourful Indian Holidays has been designing private, tailor-made journeys since 2007 — not fixed group tours with a set departure date, but itineraries built entirely around your own dates, pace and interests. Our trips cover the country broadly rather than one region: the forts and palaces of Rajasthan, the Taj Mahal and the Golden Triangle, wildlife safaris at Ranthambore and Kaziranga, the backwaters and hill country of Kerala, coastal stretches from Tamil Nadu to the Andaman Islands, and spiritual and pilgrimage circuits through Varanasi and beyond. Wherever in India a traveller wants to go, we've generally built a route there before, or are happy to plan one from scratch around what actually interests you.",
    "Timing matters as much as geography, and several of our trips are built specifically around events rather than just places — the Pushkar Camel Fair, Kumbh Mela, and Rajasthan's smaller regional festivals all shape when certain itineraries run best. We also increasingly plan honeymoon-specific trips, desert safaris, and single-region deep dives for travellers who'd rather spend real time in one place than skim several, alongside the more classic first-time circuits most visitors start with.",
    "Every trip we build includes a private, English-speaking guide and driver, handpicked heritage and luxury hotels chosen for the specific itinerary rather than a generic list, and one point of contact from your first enquiry through to the day you fly home — no handoffs between departments, no separate booking and support teams. We also plan trips that extend beyond India entirely, into Nepal and Bhutan, for travellers who want to see more of the wider subcontinent in a single journey rather than treating each country as a separate trip. Tell us where you'd like to go and what kind of trip you have in mind, and we'll build the itinerary around it.",
  ],
} as const;
